...
- [ ] 같은 클래스 내에서 `this.transactionalMethod()` 형태로 호출하는가? → self-invocation으로 트랜잭션 미적용 (별도 빈으로 분리 권고)
...
- [ ] Kafka 이벤트 페이로드 필드가 프로듀서/컨슈머 간 불일치하지 않는가? (orderId 타입, 필드명 오타)
- [ ] `@KafkaListener` 토픽명이 프로듀서의 `KafkaTemplate.send()` 토픽과 일치하는가?
...
### SSE 리소스 누수
- [ ] `onTimeout`/`onError`/`onCompletion` 3종 콜백이 모두 등록되어 있는가? (하나라도 누락 시 Emitter 누수)
- [ ] `onTimeout`/`onError`/`onCompletion` 콜백에서 Emitter가 컬렉션에서 제거되는가?
...
- [ ] `map()` 렌더링에 `key` prop이 없거나 배열 index를 사용하는가? → 고유 id(orderId, notificationId 등) 사용 권고
- [ ] `as` 타입 단언을 남용하는가? → 타입 가드 또는 타입 정의 보완 권고
