...
| `LoginPage.tsx` | (없음) | 로그인 폼. useAuth() 사용. 회원가입/계정찾기 링크 |
| `RegisterPage.tsx` | (없음) | 회원가입 폼 (이메일 포함). 비밀번호 확인 |
| `FindAccountPage.tsx` | (없음) | 탭: 아이디 찾기(이메일), 비밀번호 재설정(아이디+이메일) |
| `App.tsx` | — | 루트. 미로그인→LoginPage, 로그인→메인. 로그아웃 버튼 |

### 컨텍스트 (contexts/)
| 파일 | 기능 |
|------|------|
| `AuthContext.tsx` | AuthProvider, useAuth(). localStorage에 token/username 저장. login/register/logout. authPage 상태(login/register/find-account) |
